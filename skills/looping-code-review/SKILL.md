...
This skill orchestrates; the underlying review mechanics live in `.cursor/skills/code-review/SKILL.md` (reviewers, verifier, output format) and the local CI mechanics live in `.cursor/skills/ci-lint-test/SKILL.md`. The plan-side analogue is `/looping-plan-review`.
...
4. **Minimal changes** (per `.cursor/rules/minimal-changes/RULE.mdc`): no refactors, no renames, no "while I'm here" cleanups. Every commit in the loop must be traceable to a specific confirmed finding.
...
Invoke `code-review` (`.cursor/skills/code-review/SKILL.md`) in **review-only mode**. Its mandatory
...
2. **CI-style checks**: run the `ci-lint-test` skill (`.cursor/skills/ci-lint-test/SKILL.md`) for each changed repo — lint, format check, type check, then the relevant test suite.

...
If a plan-steps rule is accessible from where the skill is running — already loaded as an always-applied rule, or on disk at `.cursor/rules/plan-steps/RULE.mdc` under the workspace root (or an ancestor's `.cursor/rules/`) — verify the plan complies with it. If no plan-steps rule is accessible, skip this pass.
